...
namespace GPP.FSM
{
    ...
    public class Critter : MonoBehaviour
    {
        ...
        {
            StartCoroutine(Coroutines.DoOverEasedTime(transitionDuration, Easing.SineEaseOut,
                t => { GetComponent<Renderer>().material.color = Color.Lerp(Color.white, color, t); }));
        }

        // State Animations
        private IEnumerator EatingAnimation()
        {
            yield return StartCoroutine(Coroutines.DoOverEasedTime(Services.Config.EatingDuration / 2,
                Easing.BounceEaseOut,
                t => { transform.localScale = Vector3.Lerp(Vector3.one, new Vector3(1.5f, 1.5f, 1.5f), t); }));

            yield return StartCoroutine(Coroutines.DoOverEasedTime(Services.Config.EatingDuration / 2,
                Easing.BounceEaseOut,
                t => { transform.localScale = Vector3.Lerp(new Vector3(1.5f, 1.5f, 1.5f), Vector3.one, t); }));
